...
class Map:
    ...
    def get_random_point(self, top_height):
        pos_ok = False
        while not pos_ok:
            x = np.random.uniform(self.__min_x, self.__max_x)
            y = np.random.uniform(self.__min_y, self.__max_y)

            # TODO check if drone or wheeled robot. If wheeled robot then z should be near ground
            # z = np.random.uniform(0.5, top_height)
            z = top_height
