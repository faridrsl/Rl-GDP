...
class ROSInterface:
        ...
        self.__goal = Goal(
            ...
            os.path.abspath(
                os.path.expanduser(
                    ...
                    "~/ros-rl-env/catkin_ws/src/multi_robot_scenario/models/target.sdf"
                    # "~/ros-rl-env/catkin_ws/src/hector_quadrotor_noetic/hector_gazebo/hector_gazebo_worlds/Media/models/target_3/Target.sdf"
                )
            ),
        )
    ...
    def set_drone_velocity(self, linear_x, linear_y, angular_z, linear_z=0.0):
        vel_cmd = Twist()
    ...
    def respawn_robot(self, top_height=0.5):
        ...
        # for drone to hover at this altitude
        self.set_drone_velocity(linear_x=0, linear_y=0, angular_z=0, linear_z=1.0)

    def move_goal(self, x, y, z=0.25):
        self.__move_goal(x, y, z)
        self.__goal_x, self.__goal_y, self.__goal_z = x, y, z

    def reset_goal(self, goal_z_max=0):
        """
    ...
    def __reset_goal_position_static(self, goal_z_max):
        """
    ...
    def __reset_goal_position_random(self, goal_z_max):
        ...
        for _ in range(MAX_ATTEMPTS):
            ...
            # append the Z orientation to the goal _position
            yaw = y = np.random.uniform(0, 1)
            goal_position_orientation = np.append(goal_position, yaw)

            distance_to_robot = np.linalg.norm(robot_position - goal_position)

            if distance_to_robot >= MIN_DISTANCE:
                # Valid goal position found
                self.__move_goal(*goal_position_orientation)
    ...
    def __move_goal(self, x, y, z=0, yaw=0):
        """
    ...
    def __init_ROS_pubs_and_subs(self):
        ...
        self.noisy_image_pub = rospy.Publisher(
            ...
            "/front_cam/camera/image", Image, queue_size=1
        )
